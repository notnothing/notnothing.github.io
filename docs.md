...
Run `bun install` once before validating the site.

- `bun run test` runs the full validation suite.
- `bun run validate:static` checks oxfmt formatting, HTML, CSS, and Markdown.
- `bun run validate:served` starts a local static server, checks links, and runs accessibility validation.
- `bun run format` applies oxfmt formatting to website source files.
